...
from src.classifier import get_clip_model, load_classifier, predict_segment
...
class DuckAgent:
    def __init__(self, config):
        self.config = config
        self.classifier, self.label_encoder = load_classifier(config["model_path"])
...
def _run_gui(agent):
    ...
    if hasattr(gui, "run"):
        return gui.run()
    if hasattr(gui, "mainloop"):
        return gui.mainloop()
    raise RuntimeError("DuckGUI must provide run() or mainloop().")
